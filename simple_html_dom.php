...
class simple_html_dom
{
    ...
    public function mb_detect_encoding($string, $enc=null) { 
    ...
    protected function parse_charset()
    {
        ...
        if (empty($charset))
        {
            ...
            $string = $this->root->plaintext . "ascii";
            $enc = $encoding_list = array( "UTF-8", "CP1252" );
            static $list = array('utf-8', 'iso-8859-1', 'windows-1251');
        
            foreach ($list as $item) {
                $sample = iconv($item, $item, $string);
                if (md5($sample) == md5($string)) { 
                    if ($enc == $item) { $charset = true; }    else { $charset = $item; } 
                }
            }
            //return null;
